...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frame_name = name.split('/')[1]
        
        image = cv2.imread(self.cfg.datapath+'/'+self.cfg.mode+'/'+vid_name + '/JPEGImages/' + frame_name +'.jpg')[:,:,::-1].astype(np.float32)
        mask = cv2.imread(self.cfg.datapath+'/'+self.cfg.mode+'/'+vid_name + '/SegmentationClassPNG/' + frame_name + '.png', 0).astype(np.float32)
        shape = mask.shape
    ...
    def collate(self, batch):
        ...
        if self.cfg.dataset == 'VMD':
            size = 256 # Example for VMDD
        else:
            return None
