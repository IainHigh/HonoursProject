...
from config import VMD_training_root
...
parser.add_argument('--bestonly', default=True, action="store_true", help='only best model')
...
val_set = CrossPairwiseImg([VMD_training_root], val_joint_transform, img_transform, target_transform)
val_loader = DataLoader(val_set, batch_size=batch_size, num_workers=0, shuffle=False)

print("max epoch:{}".format(args['max_epoch']))

val_log_path = ckpt_path + '/val_log.txt'
with open(val_log_path, 'w') as f: f.write("")
...
def train(net, optimizer, scheduler):
    ...
    while True:
        ...
        if curr_epoch % 1 == 0 and not cmd_args.bestonly:
            ...
            torch.save(checkpoint, os.path.join(ckpt_path, f'{curr_epoch}.pth'))
