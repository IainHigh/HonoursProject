...
class VMD_Network(nn.Module):
    def __init__(
        ...
        self.ra_attention_examplar = Relation_Attention(
        ...
        initialize_weights(
            self.ra_attention_low,
            self.ra_attention_cross,
            self.ra_attention_examplar,
            self.ra_attention_query,
            self.ra_attention_other,
            self.project,
            self.final_pre,
            self.final_examplar,
            self.final_query,
    ...
    def forward(self, input1, input2, input3):
        ...
        x1 = F.interpolate(
            x1, size=low_exemplar.shape[2:], mode="bilinear", align_corners=False
        )
        x2 = F.interpolate(
            x2, size=low_query.shape[2:], mode="bilinear", align_corners=False
        )
        x3 = F.interpolate(
            other, size=low_other.shape[2:], mode="bilinear", align_corners=False
        )
        ...
        exemplar_pre = F.upsample(
            exemplar_pre, input_size, mode="bilinear", align_corners=False
        )  # upsample to the size of input image, scale=8
        query_pre = F.upsample(
            query_pre, input_size, mode="bilinear", align_corners=False
        )  # upsample to the size of input image, scale=8
        other_pre = F.upsample(
            other_pre, input_size, mode="bilinear", align_corners=False
        )  # upsample to the size of input image, scale=8

        examplar_pre_small = F.interpolate(
            exemplar_pre, size=exemplar.shape[2:], mode="bilinear", align_corners=False
        )
        query_pre_small = F.interpolate(
            query_pre, size=query.shape[2:], mode="bilinear", align_corners=False
        )
        other_pre_small = F.interpolate(
            other_pre, size=other.shape[2:], mode="bilinear", align_corners=False
        )
        ...
        outside_examplar = torch.ones(sigmoid_examplar.size()) - sigmoid_examplar
        outside_query = torch.ones(sigmoid_query.size()) - sigmoid_query
        ...
        enhanced_examplar, _ = self.ra_attention_examplar(
            sigmoid_examplar * exemplar, outside_query_feat.transpose(-2, -1)
        )
        enhanced_query, _ = self.ra_attention_query(
            sigmoid_query * query, outside_examplar_feat.transpose(-2, -1)
        )

        # enhanced_other = other
        enhanced_other, _ = self.ra_attention_other(
            sigmoid_other * other, outside_examplar_feat.transpose(-2, -1)
        )
        ...
        final_examplar = F.upsample(
            final_examplar, input_size, mode="bilinear", align_corners=False
        )  # upsample to the size of input image, scale=8
        final_query = F.upsample(
            final_query, input_size, mode="bilinear", align_corners=False
        )  # upsample to the size of input image, scale=8
        final_other = F.upsample(
            final_other, input_size, mode="bilinear", align_corners=False
        )  # upsample to the size of input image, scale=8
...
class RAttention(nn.Module):
    ...
    def forward(self, x_exmplar, x_query):
        ...
        proj_key_RL = (
            torch.diagonal(torch.transpose(proj_key, 2, 3), 0, 2, 3)
            .permute(0, 2, 1)
            .contiguous()
        )
        ...
        return (
            self.gamma_1 * (out_H + out_W + out_LR + out_RL) + x_exmplar,
            self.gamma_2 * (out_H + out_W + out_LR + out_RL) + x_query,
        )
...
class Relation_Attention(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_examplar = nn.Sequential(
        ...
        self.conv_examplar_tail = nn.Sequential(
            nn.Conv2d(inter_channels, out_channels, 3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=False),
        )
        self.conv_query_tail = nn.Sequential(
            nn.Conv2d(inter_channels, out_channels, 3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=False),
        )
...
class CoattentionModel(nn.Module):  # spatial and channel attention module
    def __init__(
        self, num_classes=1, all_channel=256, all_dim=26 * 26
    ):  # 473./8=60 416./8=52
        ...
        self.conv1 = nn.Conv2d(
            all_channel * 2, all_channel, kernel_size=3, padding=1, bias=False
        )
