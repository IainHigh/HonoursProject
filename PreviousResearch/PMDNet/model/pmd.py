...
class Contrast_Module_Deep(nn.Module):
    def __init__(self, planes, d1, d2):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.BatchNorm2d(self.inplanes_half),
            nn.ReLU(),
        )
...
class Resudial_Block(nn.Module):
    def __init__(self, in_c):
        ...
        self.conv1 = nn.Conv2d(
            in_c, out_channels=64, kernel_size=3, stride=1, padding=1
        )
...
class Refinement_Net(nn.Module):  # no localrelation
    def __init__(self, in_c):
        ...
        self.conv1 = BasicConv(
            in_planes=in_c, out_planes=64, kernel_size=3, stride=1, padding=1
        )
        # self.rl1 = LocalRelationalLayer(64)
        self.conv2 = BasicConv(
            in_planes=64, out_planes=64, kernel_size=3, stride=1, padding=1
        )
...
class PMD(nn.Module):
    def __init__(self, backbone_path=None):
        ...
        self.up_3 = nn.Sequential(
            nn.ConvTranspose2d(1024, 256, 4, 2, 1), nn.BatchNorm2d(256), nn.ReLU()
        )
        self.up_2 = nn.Sequential(
            nn.ConvTranspose2d(512, 128, 4, 2, 1), nn.BatchNorm2d(128), nn.ReLU()
        )
        self.up_1 = nn.Sequential(
            nn.ConvTranspose2d(256, 64, 4, 2, 1), nn.BatchNorm2d(64), nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        layer4_edge_feature = F.upsample(
            cbam_4, size=edge_feature.size()[2:], mode="bilinear", align_corners=True
        )
        ...
        layer4_predict = F.upsample(
            layer4_predict, size=x.size()[2:], mode="bilinear", align_corners=True
        )
        layer3_predict = F.upsample(
            layer3_predict, size=x.size()[2:], mode="bilinear", align_corners=True
        )
        layer2_predict = F.upsample(
            layer2_predict, size=x.size()[2:], mode="bilinear", align_corners=True
        )
        layer1_predict = F.upsample(
            layer1_predict, size=x.size()[2:], mode="bilinear", align_corners=True
        )

        layer0_edge = F.upsample(
            layer0_edge, size=x.size()[2:], mode="bilinear", align_corners=True
        )
        ...
        final_predict = F.upsample(
            final_predict, size=x.size()[2:], mode="bilinear", align_corners=True
        )
        ...
        return (
            F.sigmoid(layer4_predict),
            F.sigmoid(layer3_predict),
            F.sigmoid(layer2_predict),
            F.sigmoid(layer1_predict),
            F.sigmoid(layer0_edge),
            F.sigmoid(final_predict),
        )
