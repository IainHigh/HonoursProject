...
def weight_init(module):
    for n, m in module.named_children():
        ...
        if isinstance(m, nn.Conv2d):
        ...
        elif isinstance(m, (nn.BatchNorm2d, nn.InstanceNorm2d)) or isinstance(
            m, nn.GroupNorm
        ):
            ...
            or isinstance(m, nn.Softmax)
            or isinstance(m, nn.Sigmoid)
...
class basicConv(nn.Module):
    def __init__(
        ...
        conv = [
            nn.Conv2d(in_channel, out_channel, k, s, p, dilation=d, groups=g, bias=bias)
        ]
        if bn:
...
class CAM(nn.Module):
    def __init__(self, channel):
        ...
        self.down = nn.Sequential(
            conv3x3(channel, channel, stride=2), nn.BatchNorm2d(channel)
        )
        self.conv_1 = conv3x3(channel, channel)
        self.bn_1 = nn.BatchNorm2d(channel)
        self.conv_2 = conv3x3(channel, channel)
    ...
    def forward(self, x_high, x_low):
        ...
        right_1 = F.interpolate(
            x_high, size=x_low.size()[2:], mode="bilinear", align_corners=True
        )
        right_2 = x_high
        ...
        right = F.interpolate(
            right, size=x_low.size()[2:], mode="bilinear", align_corners=True
        )
...
class SAM(nn.Module):
    def __init__(self, nin: int, nout: int, num_splits: int) -> None:
        ...
        self.subspaces = nn.ModuleList(
            [
                CoordAtt(
                    int(self.nin / self.num_splits), int(self.nin / self.num_splits)
                )
...
class Net(nn.Module):
    def __init__(
        self,
        cfg,
        backbone_path="/home/iain/Desktop/HonoursProject/PreviousResearch/HetNet/resnext/resnext_101_32x4d.pth",
    ):
        ...
        self.ffm = nn.ModuleList([FFM(64), FFM(64), FFM(64), FFM(64), FFM(64)])
    ...
    def forward(self, x, shape=None):
        ...
        fused4 = F.interpolate(
            fused4, size=f5.size()[2:], mode="bilinear", align_corners=True
        )
        fused3 = self.ffm[4](f5, fused4)

        f4 = self.ca1(bk_stage4)
        fused3 = F.interpolate(
            fused3, size=f4.size()[2:], mode="bilinear", align_corners=True
        )
        ...
        fused2 = F.interpolate(
            fused2,
            size=[fused1.size(2) // 2, fused1.size(3) // 2],
            mode="bilinear",
            align_corners=True,
        )
        ...
        fused1 = F.interpolate(
            fused1, size=fused0.size()[2:], mode="bilinear", align_corners=True
        )
        ...
        edge0 = F.interpolate(
            self.head[0](fused0), size=shape, mode="bilinear", align_corners=True
        )
        out0 = F.interpolate(
            self.head[1](out0), size=shape, mode="bilinear", align_corners=True
        )

        if self.cfg.mode == "train":
            out1 = F.interpolate(
                self.head[2](fused1), size=shape, mode="bilinear", align_corners=True
            )
            out2 = F.interpolate(
                self.head[3](fused2), size=shape, mode="bilinear", align_corners=True
            )
            out3 = F.interpolate(
                self.head[4](fused3), size=shape, mode="bilinear", align_corners=True
            )
            out4 = F.interpolate(
                self.head[5](fused4), size=shape, mode="bilinear", align_corners=True
            )
