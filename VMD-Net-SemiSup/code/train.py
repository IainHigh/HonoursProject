...
combined_dataset_dir_name = "/exports/eddie/scratch/s2062378/dataset/VMD/temp_combined_dataset_SL" + str(uuid.uuid4())# Create a random directory name to avoid conflict

args = {
    'max_epoch': 10,
    'batch_size': 5,
    'self_training_start_epoch': 5,
    'self_training_confidence_threshold': 0.775, # confidence threshold for self-training
...
def process_video_batch(func_args, batch_size=256):
    ...
    with torch.no_grad():
        ...
        os.system("rm -rf %s" % (os.path.join(combined_dataset_dir_name, new_vid_name))) # Remove existing one if it exists
